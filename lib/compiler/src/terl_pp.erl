...
-import(terl_parse, [inop_prec/1,preop_prec/1,func_prec/0,max_prec/0,
                    type_inop_prec/1, type_preop_prec/1]).

-define(MAXLINE, 72).

-type(hook_function() :: none
                       | fun((Expr :: terl_parse:abstract_expr(),
...
-ifdef(DEBUG).
-define(TEST(T),
        %% Assumes that terl_anno has been compiled with DEBUG=true.
        %% terl_pp does not use the annoations, but test it anyway.
        %% Note: hooks are not handled.
        _ = try
                terl_parse:map_anno(fun(A) when is_list(A) -> A end, T)
            catch
                _:_ ->
...
-spec(attribute(Attribute) -> io_lib:chars() when
      Attribute :: terl_parse:abstract_form()).
...
-spec(function(Function) -> io_lib:chars() when
      Function :: terl_parse:abstract_form()).
...
-spec(guard(Guard) -> io_lib:chars() when
      Guard :: [terl_parse:abstract_expr()]).
...
-spec(exprs(Expressions) -> io_lib:chars() when
      Expressions :: [terl_parse:abstract_expr()]).
...
-spec(expr(Expression) -> io_lib:chars() when
      Expression :: terl_parse:abstract_expr()).
...
encoding(Options) ->
    case proplists:get_value(encoding, Options, terl_epp:default_encoding()) of
        latin1 -> latin1;
        utf8 -> unicode;
        unicode -> unicode
    end.
...
abstract(Arg, #options{encoding = Encoding}) ->
    terl_parse:abstract(Arg, [{encoding,Encoding}]).
...
lexpr({call,_,{remote,_,{atom,_,M},{atom,_,F}=N}=Name,Args}, Prec, Opts) ->
    case terl_internal:bif(M, F, length(Args)) of
        true ->
...
bit_elem_type({A,B}) ->
    [lexpr(terl_parse:abstract(A), options(none)),
     $:,
     lexpr(terl_parse:abstract(B), options(none))];
bit_elem_type(T) ->
    lexpr(terl_parse:abstract(T), options(none)).
