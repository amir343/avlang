...
terl_inputs_100 -> terl_input_top : ['$1'].
terl_inputs_100 -> terl_input_top ',' terl_inputs_100 : ['$1' | '$3'].

terl_input_top -> terl_input '|' terl_input_top
...
terl_tuple_type -> terl_type : ['$1'].
terl_tuple_type -> terl_type ',' terl_tuple_type : ['$1' | '$3'].

terl_output_top -> terl_output '|' terl_output_top
                     : {union_type, terl_union_type(['$1'], ['$3'])}.
terl_output_top -> terl_output : '$1'.

terl_output -> '(' terl_gen_type ')' : '$2'.
terl_output -> terl_type : '$1'.
...
%% Entry points compatible to old terl_parse.
...
type_tag(TypeName, NumberOfTypeVariables) ->
  case terl_internal:is_type(TypeName, NumberOfTypeVariables) of
    true -> type;
    false -> user_type
  end.

abstract2(Term, Anno) ->
  Line = terl_anno:line(Anno),
  abstract(Term, Line).
...
-spec error_bad_decl(terl_anno:anno(), attributes()) -> no_return().
...
-spec abstract(Data) -> AbsTerm when
    ...
    AbsTerm :: abstract_expr().
abstract(T) ->
  Anno = terl_anno:new(0),
  abstract(T, Anno, enc_func(terl_epp:default_encoding())).
...
abstract(T, Line) when is_integer(Line) ->
  Anno = terl_anno:new(Line),
  abstract(T, Anno, enc_func(terl_epp:default_encoding()));
abstract(T, Options) when is_list(Options) ->
  Line = proplists:get_value(line, Options, 0),
  Encoding =
    proplists:get_value(encoding, Options,terl_epp:default_encoding()),
...
new_anno(Term) ->
  map_anno(fun terl_anno:new/1, Term).

-spec anno_to_term(Abstr) -> term() when
    Abstr :: abstract_form() | abstract_expr().

anno_to_term(Abstract) ->
  map_anno(fun terl_anno:to_term/1, Abstract).

-spec anno_from_term(Term) -> abstract_form() | abstract_expr() when
    Term :: term().

anno_from_term(Term) ->
  map_anno(fun terl_anno:from_term/1, Term).

%% Forms.
%% Recognize what terl_pre_expand does:
...
%% End of terl_pre_expand special forms.
