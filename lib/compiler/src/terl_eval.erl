...
-type(expression() :: terl_parse:abstract_expr()).
-type(expressions() :: [terl_parse:abstract_expr()]).
-type(expression_list() :: [expression()]).
-type(clauses() :: [terl_parse:abstract_clause()]).
-type(name() :: term()).
-type(value() :: term()).
...
%%    or {'EXIT', Reason}
%% Only exprs/2 checks the command by calling terl_lint. The reason is
...
%% by a function handler but considered errors by terl_lint.
...
%%    or {'EXIT', Reason}
%%
%% Only expr/2 checks the command by calling terl_lint. See exprs/2.
...
%% Check a command (a list of expressions) by calling terl_lint.

check_command(Es, Bs) ->
  Opts = [bitlevel_binaries,binary_comprehension],
  case terl_lint:exprs_opt(Es, bindings(Bs), Opts) of
    {ok,_Ws} ->
      ok;
...
expr({'fun',Line,{clauses,Cs}} = Ex, Bs, Lf, Ef, RBs) ->
  ...
  {ok,Used} = terl_lint:used_vars([Ex1], Bs),
...
expr({call,_,{remote,_,Mod,Func},As0}, Bs0, Lf, Ef, RBs) ->
  ...
  case is_atom(M) andalso terl_internal:bif(M, F, length(As)) of
    true ->
      bif(F, As, Bs3, Ef, RBs);
    false ->
...
expr({call,_,{atom,_,Func},As0}, Bs0, Lf, Ef, RBs) ->
  case terl_internal:bif(Func, length(As0)) of
    true ->
...
hide({call,L,{atom,_,N}=Atom,Args}, Id0, D0) ->
  ...
  C = case terl_internal:bif(N, length(Args)) of
        true ->
          {call,L,Atom,NArgs};
        false ->
          A = terl_anno:new(Id),
...
eval_filter(F, Bs0, Lf, Ef, CompFun, Acc) ->
  case terl_lint:is_guard_test(F) of
    true ->
...
guard0([G|Gs], Bs0, Lf, Ef) ->
  case terl_lint:is_guard_test(G) of
    true ->
...
%%    true = terl_internal:guard_bif(F, length(As)),
