...
-spec bool_option(atom(), atom(), boolean(), [terl_compile:option()]) -> boolean().
...
%%
%% Used by terl_eval.erl to check commands.
%%
...
-spec(module(AbsForms) -> {ok, Warnings} | {error, Errors, Warnings} when
      AbsForms :: [terl_parse:abstract_form()],
...
-spec(module(AbsForms, FileName, CompileOptions) ->
      ...
      AbsForms :: [terl_parse:abstract_form()],
      FileName :: atom() | string(),
      CompileOptions :: [terl_compile:option()],
      Warnings :: [{file:filename(),[ErrorInfo]}],
      Errors :: [{FileName2 :: file:filename(),[ErrorInfo]}],
      ErrorInfo :: error_info()).
...
add_error(Anno, E, St) ->
    ...
    add_lint_error({Location,terl_lint,E}, File, St).
...
add_warning(FileLine, W, St) ->
    ...
    add_lint_warning({Location,terl_lint,W}, File, St).

add_lint_warning(W, File, St) ->
    St#lint{warnings=[{File,W}|St#lint.warnings]}.

loc(Anno, St) ->
    Location = terl_anno:location(Anno),
    case terl_anno:file(Anno) of
...
includes_qlc_hrl(Forms, St) ->
    %% QLC calls terl_lint several times, sometimes with the compile
...
set_form_file({attribute,L,K,V}, File) ->
    {attribute,terl_anno:set_file(File, L),K,V};
set_form_file({function,L,N,A,C}, File) ->
    {function,terl_anno:set_file(File, L),N,A,C};
set_form_file(Form, _File) ->
    Form.

set_file(T, File) ->
    F = fun(Anno) -> terl_anno:set_file(File, Anno) end,
    terl_parse:map_anno(F, T).
...
disallowed_compile_flags(Forms, St0) ->
    ...
    Errors0 =  [ {St0#lint.file,{L,terl_lint,disallowed_nowarn_bif_clash}} ||
                   {attribute,A,compile,nowarn_bif_clash} <- Forms,
                   {_,L} <- [loc(A, St0)] ],
    Errors1 = [ {St0#lint.file,{L,terl_lint,disallowed_nowarn_bif_clash}} ||
                  {attribute,A,compile,{nowarn_bif_clash, {_,_}}} <- Forms,
                  {_,L} <- [loc(A, St0)] ],
    Disabled = (not is_warn_enabled(bif_clash, St0)),
    Errors = if
                 Disabled andalso Errors0 =:= [] ->
                     [{St0#lint.file,{terl_lint,disallowed_nowarn_bif_clash}} | St0#lint.errors];
                 Disabled ->
        ...
        Efs ->
            {Err, St1} =
                foldl(fun ({bif,{F,A},_}, {Err,St0}) ->
                              ...
                              OldBif = terl_internal:old_bif(F,A),
                              {Err,if
...
-spec on_load(terl_anno:anno(), fa(), lint_state()) -> lint_state().
...
call_function(Line, F, A, #lint{usage=Usage0,called=Cd,func=Func,file=File}=St) ->
    ...
    Anno = terl_anno:set_file(File, Line),
...
is_pattern_expr(Expr) ->
    case is_pattern_expr_1(Expr) of
        ...
        true ->
            ...
            case terl_eval:partial_eval(Expr) of
                {integer,_,_} -> true;
                {char,_,_} -> true;
                {float,_,_} -> true;
                {atom,_,_} -> true;
...
is_pattern_expr_1({op,_Line,Op,A}) ->
    terl_internal:arith_op(Op, 1) andalso is_pattern_expr_1(A);
is_pattern_expr_1({op,_Line,Op,A1,A2}) ->
    terl_internal:arith_op(Op, 2) andalso all(fun is_pattern_expr/1, [A1,A2]);
...
pat_bit_size(Size, _Vt, _Bvt, St) ->
    ...
    case is_pattern_expr(Size) of
        true ->
            case terl_eval:partial_eval(Size) of
                {integer,Line,I} -> {I,[],[],St};
...
bit_type(Line, Size0, Type, St) ->
    case terl_bits:set_bit_type(Size0, Type) of
        {ok,Size1,Bt} -> {Size1,Bt,St};
        {error,What} ->
            %% Flag error and generate a default.
            {ok,Size1,Bt} = terl_bits:set_bit_type(default, []),
...
guard_test2({call,Line,{atom,_La,F},As}=G, Vt, St0) ->
    ...
    A = length(As),
    case terl_internal:type_test(F, A) of
...
gexpr({call,Line,{atom,_La,F},As}, Vt, St0) ->
    ...
    case terl_internal:guard_bif(F, A) andalso no_guard_bif_clash(St1,{F,A}) of
        true ->
            %% Assert that it is auto-imported.
            true = terl_internal:bif(F, A),
            {Asvt,St1};
        false ->
...
gexpr({call,Line,{remote,_Lr,{atom,_Lm,erlang},{atom,_Lf,F}},As}, Vt, St0) ->
    {Asvt,St1} = gexpr_list(As, Vt, St0),
    A = length(As),
    case terl_internal:guard_bif(F, A) orelse is_gexpr_op(F, A) of
        true -> {Asvt,St1};
...
is_guard_test2({call,_Line,{atom,_La,Test},As}=Call, RDs) ->
    case terl_internal:type_test(Test, length(As)) of
...
is_gexpr({call,_L,{atom,_Lf,F},As}, RDs) ->
    A = length(As),
    terl_internal:guard_bif(F, A) andalso is_gexpr_list(As, RDs);
is_gexpr({call,_L,{remote,_Lr,{atom,_Lm,erlang},{atom,_Lf,F}},As}, RDs) ->
    A = length(As),
    (terl_internal:guard_bif(F, A) orelse is_gexpr_op(F, A))
...
is_gexpr_op(Op, A) ->
    try terl_internal:op_type(Op, A) of
        arith -> true;
        bool  -> true;
        comp  -> true;
        list  -> false;
...
expr({call,Line,{atom,La,F},As}, Vt, St0) ->
    ...
    IsAutoBif = terl_internal:bif(F, A),
                              ...
                              N ->
                                  ...
                                  St3 = if
                                            (not AutoSuppressed) andalso IsAutoBif andalso Warn ->
                                                case terl_internal:old_bif(F,A) of
                                                    true ->
                                                        add_error
                                                          (Line,
...
ginit_fields(Ifs, Line, Name, Dfs, Vt0, St0) ->
    ...
    IllErrs = [E || {_File,{_Line,terl_lint,illegal_guard_expr}}=E <- Errors],
...
check_type({type, L, range, [From, To]}, SeenVars, St) ->
    St1 =
        case {terl_eval:partial_eval(From), terl_eval:partial_eval(To)} of
...
check_type({type, L, binary, [Base, Unit]}, SeenVars, St) ->
    St1 =
        case {terl_eval:partial_eval(Base), terl_eval:partial_eval(Unit)} of
            {{integer, _, BaseVal},
...
check_type(I, SeenVars, St) ->
    case terl_eval:partial_eval(I) of
...
used_type(TypePair, L, #lint{usage = Usage, file = File} = St) ->
    ...
    UsedTypes = dict:store(TypePair, terl_anno:set_file(File, L), OldUsed),
    St#lint{usage=Usage#usage{used_types=UsedTypes}}.

is_default_type({Name, NumberOfTypeVariables}) ->
    terl_internal:is_type(Name, NumberOfTypeVariables).
...
nowarn() ->
    A0 = terl_anno:new(0),
    A1 = terl_anno:set_generated(true, A0),
    terl_anno:set_file("", A1).
...
vtunsafe({Tag,FileLine}, Uvt, Vt) ->
    Line = terl_anno:location(FileLine),
...
copy_expr(Expr, Anno) ->
    terl_parse:map_anno(fun(_A) -> Anno end, Expr).
...
obsolete_guard({call,Line,{atom,Lr,F},As}, St0) ->
    Arity = length(As),
    case terl_internal:old_type_test(F, Arity) of
        false ->
