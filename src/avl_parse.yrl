...
expr expr_90 expr_100 expr_150 expr_160 expr_200 expr_300 expr_400 expr_500
...
avl_fun_type avl_fun_type_100 avl_type_alias avl_gen_type avl_list_type
avl_inputs avl_inputs_100 avl_input avl_input_top avl_type avl_tuple_type
avl_output avl_output_top avl_type_cons avl_type_params avl_cons_params
avl_cons_param avl_record_type.
...
form -> function dot : '$1'.
form -> avl_fun_type : '$1'.
form -> avl_type_alias : '$1'.
form -> avl_type_cons : '$1'.
form -> avl_record_type : '$1'.
...
avl_type_alias -> atom var '::' avl_output_top dot
                     : mk_type_alias('$1', '$2', '$4').

avl_record_type -> atom '#' atom '::' '{' avl_tuple_type '}' dot
                      : mk_record_type('$1', '$3', '$6').

avl_fun_type -> atom '::' avl_fun_type_100 dot
                   : avl_build_fun_sig('$1', '$3').
avl_fun_type -> atom ':' atom '::' avl_fun_type_100 dot
                   : avl_build_remote_fun_sig('$1', '$3', '$5').

avl_fun_type_100 -> avl_gen_type : ['$1'].
avl_fun_type_100 -> avl_gen_type ';' avl_fun_type_100 : ['$1' | '$3'].

avl_type_cons -> atom var '(' avl_cons_params ')' '::' avl_output_top dot
                    : mk_type_cons('$1', '$2', '$4', '$7').

avl_cons_params -> avl_cons_param ',' avl_cons_params : ['$1' | '$3'].
avl_cons_params -> avl_cons_param : ['$1'].

avl_cons_param -> var : mk_type_cons_param('$1').

avl_gen_type -> avl_inputs '->' avl_output_top : {fun_type, '$1', '$3'}.

avl_inputs -> '(' ')' : [].
avl_inputs -> '(' avl_inputs_100 ')' : '$2'.
avl_inputs -> avl_inputs_100 : '$1'.

avl_inputs_100 -> avl_input_top : ['$1'].
avl_inputs_100 -> avl_input_top ',' avl_inputs_100 : ['$1' | '$3'].

avl_input_top -> avl_input '|' avl_input_top
                    : {union_type, avl_union_type(['$1'], ['$3'])}.
avl_input_top -> avl_input : '$1'.

avl_input -> '(' avl_fun_type_100 ')' : '$2'.
avl_input -> avl_type : '$1'.

avl_type -> '(' '..' ')' '->' '..' : {untyped_fun, nil, nil}.
avl_type -> '[' avl_list_type ']' : {list_type, '$2'}.
avl_type -> '[' ']' : {list_type, nothing}.
avl_type -> '{' '}' : {tuple_type, []}.
avl_type -> '{' avl_tuple_type '}' : {tuple_type, '$2'}.
avl_type -> '#' atom '{' '}' : avl_record_type('$2').
avl_type -> var : avl_build_type(element(3, '$1')).
avl_type -> atom : {avl_atom_type, element(3, '$1')}.
avl_type -> atom ':' var
               : {avl_type_ref, element(3, '$1'), element(3, '$3')}.
avl_type -> var '(' avl_type_params ')'
               : {type_instance, element(3, '$1'), '$3'}.

avl_type_params -> avl_type : ['$1'].
avl_type_params -> avl_type ',' avl_type_params : ['$1' | '$3'].
avl_type_params -> avl_gen_type : ['$1'].
avl_type_params -> avl_gen_type ',' avl_type_params : ['$1' | '$3' ].

avl_list_type -> '(' avl_output_top ')': '$2'.
avl_list_type -> avl_type : '$1'.

avl_tuple_type -> '(' avl_gen_type ')' : ['$2'].
avl_tuple_type -> '(' avl_gen_type ')' ',' avl_tuple_type : ['$2' | '$5'].
avl_tuple_type -> avl_type : ['$1'].
avl_tuple_type -> avl_type ',' avl_tuple_type : ['$1' | '$3'].

avl_output_top -> avl_output '|' avl_output_top
                     : {union_type, avl_union_type(['$1'], ['$3'])}.
avl_output_top -> avl_output : '$1'.

avl_output -> '(' avl_gen_type ')' : '$2'.
avl_output -> avl_type : '$1'.
...
typed_expr -> expr ':' avl_output_top  : {typed, '$1', '$3'}.
...
expr_90 -> atom var '::' avl_output_top
             : avl_typed_annotation('$1', '$2', '$4').

expr_90 -> atom var '::' '(' avl_fun_type_100 ')'
             : avl_typed_annotation('$1', '$2', '$5').

expr_90 -> var '::' '(' avl_fun_type_100 ')' '=' expr_150
             : avl_make_typed_match(?anno('$6'), '$1', '$4', '$7').

expr_90 -> var '::' avl_output_top '=' expr_150
             : avl_make_typed_match(?anno('$4'),'$1','$3','$5').
expr_90 -> expr_100 : '$1'.
...
-type error_info() :: {avl_anno:line(), module(), error_description()}.
-type token() :: avl_scan:token().
...
%% Entry points compatible to old avl_parse.
...
type_tag(TypeName, NumberOfTypeVariables) ->
  case avl_internal:is_type(TypeName, NumberOfTypeVariables) of
    true -> type;
    false -> user_type
  end.

abstract2(Term, Anno) ->
  Line = avl_anno:line(Anno),
  abstract(Term, Line).
...
-spec error_bad_decl(avl_anno:anno(), attributes()) -> no_return().
...
abstract(T) ->
  Anno = avl_anno:new(0),
  abstract(T, Anno, enc_func(avl_epp:default_encoding())).
...
abstract(T, Line) when is_integer(Line) ->
  Anno = avl_anno:new(Line),
  abstract(T, Anno, enc_func(avl_epp:default_encoding()));
abstract(T, Options) when is_list(Options) ->
  Line = proplists:get_value(line, Options, 0),
  Encoding =
    proplists:get_value(encoding, Options,avl_epp:default_encoding()),
...
set_line(L, F) ->
  avl_scan:set_attribute(line, L, F).

get_attribute(L, Name) ->
  avl_scan:attributes_info(L, Name).

get_attributes(L) ->
  avl_scan:attributes_info(L).
...
new_anno(Term) ->
  map_anno(fun avl_anno:new/1, Term).

-spec anno_to_term(Abstr) -> term() when
    Abstr :: abstract_form() | abstract_expr().

anno_to_term(Abstract) ->
  map_anno(fun avl_anno:to_term/1, Abstract).

-spec anno_from_term(Term) -> abstract_form() | abstract_expr() when
    Term :: term().

anno_from_term(Term) ->
  map_anno(fun avl_anno:from_term/1, Term).

%% Forms.
%% Recognize what avl_pre_expand does:
...
%% End of avl_pre_expand special forms.
...
avl_build_fun_sig({atom, L, Name}, Ts) ->
  {fun_sig, L, Name, Ts}.

avl_build_remote_fun_sig({atom, L, Module}, {atom, L, Fun}, Ts) ->
  {fun_remote_sig, L, Module, Fun, Ts}.

avl_build_type(Tag) ->
  type_internal:type_tag(Tag).

avl_union_type(Types, [R]) when is_tuple(R) ->
  case R of
    {union_type, T} -> Types ++ T;
    _ -> Types ++ [R]
  end;
avl_union_type(Types, T) ->
  Types ++ T.

avl_record_type({atom, L, Name}) ->
...
avl_typed_annotation(A, Name, Def) ->
  {atom, L, Atom} = A,
  case Atom of
    type ->
    ...
    true ->
      return_error(L,
                   io_lib:format(
                     ...
                     ++  " parameter '~w'", [Var]));
    false ->
      {avl_generic_type, Var}
  end.

avl_make_typed_match(Line, Var, Type, Expr) ->
  case Type of
    {fun_type, _, _} ->
