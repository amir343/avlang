...
-type(expression() :: avl_parse:abstract_expr()).
-type(expressions() :: [avl_parse:abstract_expr()]).
-type(expression_list() :: [expression()]).
-type(clauses() :: [avl_parse:abstract_clause()]).
-type(name() :: term()).
-type(value() :: term()).
...
%%    or {'EXIT', Reason}
%% Only exprs/2 checks the command by calling avl_lint. The reason is
...
%% by a function handler but considered errors by avl_lint.
...
%%    or {'EXIT', Reason}
%%
%% Only expr/2 checks the command by calling avl_lint. See exprs/2.
...
%% Check a command (a list of expressions) by calling avl_lint.

check_command(Es, Bs) ->
  Opts = [bitlevel_binaries,binary_comprehension],
  case avl_lint:exprs_opt(Es, bindings(Bs), Opts) of
    {ok,_Ws} ->
      ok;
...
expr({'fun',_Line,{function,Name,Arity}}, _Bs0, _Lf, _Ef, _RBs) -> % R8
  ...
  erlang:raise(error, undef, [{avl_eval,Name,Arity}|stacktrace()]);
expr({'fun',Line,{clauses,Cs}} = Ex, Bs, Lf, Ef, RBs) ->
  ...
  {ok,Used} = avl_lint:used_vars([Ex1], Bs),
...
expr({call,_,{remote,_,Mod,Func},As0}, Bs0, Lf, Ef, RBs) ->
  ...
  case is_atom(M) andalso avl_internal:bif(M, F, length(As)) of
    true ->
      bif(F, As, Bs3, Ef, RBs);
    false ->
...
expr({call,_,{atom,_,Func},As0}, Bs0, Lf, Ef, RBs) ->
  case avl_internal:bif(Func, length(As0)) of
    true ->
...
hide({call,L,{atom,_,N}=Atom,Args}, Id0, D0) ->
  ...
  C = case avl_internal:bif(N, length(Args)) of
        true ->
          {call,L,Atom,NArgs};
        false ->
          A = avl_anno:new(Id),
...
local_func(Func, As0, _Bs0, none, _Ef, _RBs) ->
  erlang:raise(error, undef, [{avl_eval,Func,length(As0)}|stacktrace()]).

local_func2({value,V,Bs}, RBs) ->
  ret_expr(V, Bs, RBs);
local_func2({eval,F,As,Bs}, RBs) -> % This reply is not documented.
  %% The shell found F. avl_eval tries to do a tail recursive call,
...
eval_filter(F, Bs0, Lf, Ef, CompFun, Acc) ->
  case avl_lint:is_guard_test(F) of
    true ->
...
guard0([G|Gs], Bs0, Lf, Ef) ->
  case avl_lint:is_guard_test(G) of
    true ->
