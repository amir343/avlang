...
-record(core, {vcount=0 :: non_neg_integer(),	%Variable counter
               ...
               opts     :: [avl_compile:option()],	%Options.
               ws=[]    :: [warning()],		%Warnings.
               file=[{file,""}]}).              %File
...
-spec module({module(), [fa()], [form()]}, [avl_compile:option()]) ->
...
attribute(Attribute) ->
  Fun = fun(A) ->  [avl_anno:location(A)] end,
  {attribute,Line,Name,Val} = avl_parse:map_anno(Fun, Attribute),
...
gexpr({op,Line,Op,L,R}=E, Bools, St) ->
  case avl_internal:bool_op(Op, 2) of
    true ->
...
gexpr_test(E0, Bools0, St0) ->
  ...
  case E1 of
    #icall{anno=Anno,module=#c_literal{val=erlang},name=#c_literal{val=N},args=As} ->
	    Ar = length(As),
	    case avl_internal:type_test(N, Ar) orelse
        avl_internal:comp_op(N, Ar) orelse
        avl_internal:bool_op(N, Ar) of
...
constant_bin_1(Es) ->
  ...
  EmptyBindings = avl_eval:new_bindings(),
...
%%  Test if a general expression is a guard test.  Use avl_lint here
%%  as it now allows avl_pre_expand transformed source.

is_guard_test(E) -> avl_lint:is_guard_test(E).
...
record_anno(L, St) ->
  case
    avl_anno:record(L) andalso member(dialyzer, St#core.opts)
  of
    true ->
...
lineno_anno(L, St) ->
  Line = avl_anno:line(L),
  Generated = avl_anno:generated(L),
...
no_compiler_warning(Anno) ->
  avl_anno:set_generated(true, Anno).
...
add_warning(Anno, Term, #core{ws=Ws,file=[{file,File}]}=St) ->
  case avl_anno:generated(Anno) of
    false ->
      St#core{ws=[{File,[{avl_anno:location(Anno),?MODULE,Term}]}|Ws]};
    true ->
      St
  end.
