...
-spec column(Token) -> avl_anno:column() | 'undefined' when
      Token :: token().

column(Token) ->
    avl_anno:column(element(2, Token)).

-spec end_location(Token) -> avl_anno:location() | 'undefined' when
      Token :: token().

end_location(Token) ->
    avl_anno:end_location(element(2, Token)).

-spec line(Token) -> avl_anno:line() when
      Token :: token().

line(Token) ->
    avl_anno:line(element(2, Token)).

-spec location(Token) -> avl_anno:location() when
      Token :: token().

location(Token) ->
    avl_anno:location(element(2, Token)).

-spec text(Token) -> avl_anno:text() | 'undefined' when
      Token :: token().

text(Token) ->
    avl_anno:text(element(2, Token)).
