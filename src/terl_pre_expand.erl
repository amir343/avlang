...
module(Fs0, Opts0) ->
    ...
    Fs = terl_expand_records:module(Fs0, Opts0),
...
module_predef_funcs(St) ->
    ...
    Mpf = [terl_parse:new_anno(F) || F <- Mpf1++Mpf2],
    {Mpf,St2}.
...
expr({call,Line,{atom,La,N}=Atom,As0}, St0) ->
	...
	_ ->
		...
		no ->
		    case terl_internal:bif(N, Ar) of
			true ->
			    {{call,Line,{remote,La,{atom,La,erlang},Atom},As},St1};
			false -> %% This should have been handled by terl_lint
			    {{call,Line,Atom,As},St1}
		    end
	    end
    end;
...
lc_tq(Line, [F0 | Qs0], St0) ->
    case terl_lint:is_guard_test(F0) of
        true ->
...
%% name of a BIF (terl_lint has checked that it is not an import).
%% "Implicit" funs {'fun', Line, {function, F, A}} are not changed.

fun_tq(Lf, {function,F,A}=Function, St0) ->
    case terl_internal:bif(F, A) of
        true ->
...
pat_bit_size(Size, St) ->
    Line = element(2, Size),
    {value,Sz,_} = terl_eval:expr(Size, terl_eval:new_bindings()),
    {{integer,Line,Sz},St}.

make_bit_type(Line, default, Type0) ->
    case terl_bits:set_bit_type(default, Type0) of
        {ok,all,Bt} -> {{atom,Line,all},terl_bits:as_list(Bt)};
	{ok,undefined,Bt} -> {{atom,Line,undefined},terl_bits:as_list(Bt)};
        {ok,Size,Bt} -> {{integer,Line,Size},terl_bits:as_list(Bt)}
    end;
make_bit_type(_Line, Size, Type0) ->            %Integer or 'all'
    {ok,Size,Bt} = terl_bits:set_bit_type(Size, Type0),
    {Size,terl_bits:as_list(Bt)}.
