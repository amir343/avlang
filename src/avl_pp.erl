...
-import(avl_parse, [inop_prec/1,preop_prec/1,func_prec/0,max_prec/0,
                    type_inop_prec/1, type_preop_prec/1]).

-define(MAXLINE, 72).

-type(hook_function() :: none
                       | fun((Expr :: avl_parse:abstract_expr(),
...
-ifdef(DEBUG).
-define(TEST(T),
        %% Assumes that avl_anno has been compiled with DEBUG=true.
        %% avl_pp does not use the annoations, but test it anyway.
        %% Note: hooks are not handled.
        _ = try
                avl_parse:map_anno(fun(A) when is_list(A) -> A end, T)
            catch
                _:_ ->
...
-spec(attribute(Attribute) -> io_lib:chars() when
      Attribute :: avl_parse:abstract_form()).
...
-spec(function(Function) -> io_lib:chars() when
      Function :: avl_parse:abstract_form()).
...
-spec(exprs(Expressions) -> io_lib:chars() when
      Expressions :: [avl_parse:abstract_expr()]).
...
-spec(expr(Expression) -> io_lib:chars() when
      Expression :: avl_parse:abstract_expr()).
...
encoding(Options) ->
    case proplists:get_value(encoding, Options, avl_epp:default_encoding()) of
        latin1 -> latin1;
        utf8 -> unicode;
        unicode -> unicode
    end.
...
abstract(Arg, #options{encoding = Encoding}) ->
    avl_parse:abstract(Arg, [{encoding,Encoding}]).
...
lexpr({call,_,{remote,_,{atom,_,M},{atom,_,F}=N}=Name,Args}, Prec, Opts) ->
    case avl_internal:bif(M, F, length(Args)) of
        true ->
...
bit_elem_type({A,B}) ->
    [lexpr(avl_parse:abstract(A), options(none)),
     $:,
     lexpr(avl_parse:abstract(B), options(none))];
bit_elem_type(T) ->
    lexpr(avl_parse:abstract(T), options(none)).
