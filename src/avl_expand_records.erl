...
expr({nil,_}=Nil, St) ->
    {Nil,St};
expr({type_anno, _, _, _}=TypeAnno, St) ->
    {TypeAnno, St};
...
strict_get_record_field(Line, R, {atom,_,F}=Index, Name, St0) ->
    case is_in_guard() of
        ...
        true ->                                 %In a guard.
            ...
            A0 = avl_anno:new(0),
            ExpRp = avl_parse:map_anno(fun(_A) -> A0 end, ExpR),
...
mark_record(Anno, St) ->
    case lists:member(dialyzer, St#exprec.compile) of
        true -> avl_anno:set_record(true, Anno);
        false -> Anno
    end.
