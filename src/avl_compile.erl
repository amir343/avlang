...
-type err_info() :: {avl_anno:line() | 'none',
...
env_default_opts() ->
  ...
  case os:getenv(Key) of
    ...
    Str when is_list(Str) ->
      case avl_scan:string(Str) of
        {ok,Tokens,_} ->
          Dot = {dot, avl_anno:new(1)},
          case avl_parse:parse_term(Tokens ++ [Dot]) of
...
expand_module(#compile{code=Code,options=Opts0}=St0) ->
  {Mod,Exp,Forms,Opts1} = avl_pre_expand:module(Code, Opts0),
...
abstract_code(#compile{code=Code0,options=Opts,ofile=OFile}) ->
  Code = avl_parse:anno_to_term(Code0),
...
do_src_listing(Lf, Fs) ->
  ...
  foreach(fun (F) -> io:put_chars(Lf, [avl_pp:form(F, Opts),"\n"]) end,
          Fs).
...
output_encoding(F, #compile{encoding = none}) ->
  ok = io:setopts(F, [{encoding, avl_epp:default_encoding()}]);
output_encoding(F, #compile{encoding = Encoding}) ->
  ok = io:setopts(F, [{encoding, Encoding}]),
  ok = io:fwrite(F, <<"%% ~s\n">>, [avl_epp:encoding_to_string(Encoding)]).

restore_expanded_types("P", Fs) ->
  avl_epp:restore_typed_record_fields(Fs);
restore_expanded_types("E", {M,I,Fs0}) ->
  Fs1 = restore_expand_module(Fs0),
  Fs = avl_epp:restore_typed_record_fields(Fs1),
  {M,I,Fs};
