...
-spec parse_erl_form(Epp) ->
      ...
      Epp :: epp_handle(),
      AbsForm :: terl_parse:abstract_form(),
      Line :: terl_anno:line(),
      ErrorInfo :: terl_scan:error_info() | terl_parse:error_info().

parse_erl_form(Epp) ->
    case epp_request(Epp, scan_erl_form) of
	{ok,Toks} ->
	    terl_parse:parse_form(Toks);
	Other ->
	    Other
    end.
...
-spec parse_file(FileName, IncludePath, PredefMacros) ->
      ...
      FileName :: file:name(),
      IncludePath :: [DirectoryName :: file:name()],
      Form :: terl_parse:abstract_form() | {'error', ErrorInfo} | {'eof',Line},
      PredefMacros :: macros(),
      Line :: terl_anno:line(),
      ErrorInfo :: terl_scan:error_info() | terl_parse:error_info(),
...
user_predef([{M,Val,redefine}|Pdm], Ms) when is_atom(M) ->
    Exp = terl_parse:tokens(terl_parse:abstract(Val)),
...
enter_file_reply(From, Name, LocationAnno, AtLocation, Where) ->
    ...
    Anno = case Where of
               code -> Anno0;
               generated -> terl_anno:set_generated(true, Anno0)
           end,
...
%% terl_scan:text(T) is not backward compatible with this.
...
interpret_file_attr([{attribute,Anno,file,{File,Line}}=Form | Forms],
                    Delta, Fs) ->
    L = get_line(Anno),
    Generated = terl_anno:generated(Anno),
    if
        Generated ->
            %% -file attribute
...
interpret_file_attr([Form0 | Forms], Delta, Fs) ->
    F = fun(Anno) ->
                ...
                terl_anno:set_line(Line + Delta, Anno)
        end,
    Form = terl_parse:map_anno(F, Form0),
