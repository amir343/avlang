...
-spec column(Token) -> terl_anno:column() | 'undefined' when
      Token :: token().

column(Token) ->
    terl_anno:column(element(2, Token)).

-spec end_location(Token) -> terl_anno:location() | 'undefined' when
      Token :: token().

end_location(Token) ->
    terl_anno:end_location(element(2, Token)).

-spec line(Token) -> terl_anno:line() when
      Token :: token().

line(Token) ->
    terl_anno:line(element(2, Token)).

-spec location(Token) -> terl_anno:location() when
      Token :: token().

location(Token) ->
    terl_anno:location(element(2, Token)).

-spec text(Token) -> terl_anno:text() | 'undefined' when
      Token :: token().

text(Token) ->
    terl_anno:text(element(2, Token)).
...
attributes_info(Attrs, column=Item) ->
    case attr_info(Attrs, Item) of
        undefined ->
            case terl_anno:column(Attrs) of
                undefined ->
                    undefined;
                Column ->
                    {Item,Column}
...
anno(Line, no_col, #erl_scan{text = true}, String) ->
    Anno = anno(Line),
    terl_anno:set_text(String, Anno);
