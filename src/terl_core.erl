...
-record(core, {vcount=0 :: non_neg_integer(),	%Variable counter
               ...
               opts     :: [terl_compile:option()],	%Options.
               ws=[]    :: [warning()],		%Warnings.
               file=[{file,""}]}).              %File
...
-spec module({module(), [fa()], [form()]}, [terl_compile:option()]) ->
...
gexpr({op,Line,Op,L,R}=E, Bools, St) ->
  case terl_internal:bool_op(Op, 2) of
    true ->
...
gexpr_test(E0, Bools0, St0) ->
  ...
  case E1 of
    #icall{anno=Anno,module=#c_literal{val=erlang},name=#c_literal{val=N},args=As} ->
	    Ar = length(As),
	    case terl_internal:type_test(N, Ar) orelse
        terl_internal:comp_op(N, Ar) orelse
        terl_internal:bool_op(N, Ar) of
...
constant_bin_1(Es) ->
  ...
  EmptyBindings = terl_eval:new_bindings(),
...
%%  Test if a general expression is a guard test.  Use terl_lint here
%%  as it now allows terl_pre_expand transformed source.

is_guard_test(E) -> terl_lint:is_guard_test(E).
